...
export const processDocument = inngest.createFunction(
  ...
  async ({ event, step }: any) => {
    ...
    try {
      ...
      console.error(`[ingest] Ingestion FAILED for document ${documentId}:`, err?.message || err);
      try {
        await supabaseAdmin
          .from('documents')
          .update({
        ...
        console.error(`[ingest] Failed to mark document ${documentId} as FAILED in Postgres:`, dbErr?.message || dbErr);
      }
      throw err; // rethrow so Inngest registers the failure / retry appropriately
    }
  }
);
